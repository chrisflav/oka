...
* **Local isomorphisms and finite étale morphisms** (`Oka/AnalyticSpace/LocalIso.lean`), the
  ...
  *the pullback along the inclusion of an open subspace is in
  ...
  neither is enough for an axiom that quantifies over every cospan*, until later the same day,
  when `Oka/AnalyticSpace/CutOutFibreProduct.lean` added the third. **Conservativity is one of the
  axioms a Galois category asks of a fibre functor, and faithfulness is not one of them**: at the
  Mathlib revision `lakefile.toml` pins, the class there asks the functor to reflect isomorphisms
  and *derives* faithfulness from the axioms rather than assuming it. So what is here is that
  axiom and something such a structure hands back, and not the structure.
...
```lean
#lint only docBlame    in Oka
#lint only docBlameThm in Oka
```

The `in <package>` form selects by package rather than by file, and `only` runs a disabled linter
without patching Batteries. At `edc89d9` the second reports **428 findings in 54 of the 123 files**
under `Oka/`, against the first's **0** over the same 1838 declarations — which is what makes the
428 mean something rather than being a number from an unvalidated instrument. Three files carry
**189** of it: `Oka/Weierstrass.lean` 79, `Oka/LocalOkaRing.lean` 67 and
`Oka/Geometry/RingedSpace/PresheafedSpace/Gluing.lean` 43. The other 239 are spread over 51
modules, median under five.

**428 is not 428 docstrings to write.** 354 are declarations somebody wrote. The other 74 are
generated, in five groups that behave differently — each established with a compiled control
carrying a documented *and* an undocumented case, because none of these is guessable:

| generated by | count | what it costs |
|---|---|---|
| `@[reassoc]`, the `…_assoc` lemmas | 15 | **nothing** — each clears when its base is written |
| anonymous instances of `Prop` classes | 28 | one docstring each, **and no rename** |
| `Prop`-valued fields of a `structure` or `class` | 7 | one docstring each, **on the field** |
| `@[ext]`, the `.ext` and `.ext_iff` lemmas | 6 | **irreducible** |
| `@[simps]` and `@[mk_iff]` | 18 | **irreducible** |

Row by row, and each of these is a control and not a reading of the linter's source: all 15
`…_assoc` bases are themselves among the 428, so writing the bases clears both. An anonymous
instance takes a `/-- … -/` and the environment keeps it, so those 28 need no rename. A documented
`Prop` field silences its projection and a bare one does not. **Documenting the fields does not
silence `.ext` or `.ext_iff`, and documenting a definition does not silence its `@[simps]`
lemmas** — those two rows are why "irreducible" is the right word.
