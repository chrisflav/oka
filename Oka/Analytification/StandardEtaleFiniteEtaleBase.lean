...
* **No unrestricted `ComplexAnalytic.AnalyticSpace.IsFiniteEtale`**, and there never will be: it
  is false, and `Oka/Analytification/MonicHypersurface.lean` carries the witness.
* **No instance of the class below.** Nothing in this file instantiates anything — the second
  hypothesis of the finiteness half, an open subset of `ℂ^n` avoiding the bad set, is a condition
  on the pair that nothing below bounds. What is proved nowhere is a `k ≥ 1` pair at which the
  open subset is **proper** and non-empty, which is the only case in which the restriction both
  removes something and leaves something.
